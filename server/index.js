...
// MongoDB Connection
mongoose.connect(process.env.MONGODB_URI || 'mongodb://127.0.0.1:27017/solarApp')
...
app.post('/api/save-calculation', authenticateToken, async (req, res) => {
    ...
    if (!calculationParams || !resultData) return res.status(400).json({ message: 'Missing calculation parameters or results.' });
    try {
...
async function getCoordinates(location) {
    ...
    try {
        ...
        if (error.response) throw new Error(`Geocoding service error: ${error.response.status} ${error.response.statusText}`);
...
async function getSolarIrradiance(lat, lon, tilt, azimuth, shadingLoss) {
    ...
    try {
        ...
        if (error.response) throw new Error(`PVGIS API Error: ${error.response.status} ${error.response.data?.message || error.response.statusText}`);
...
app.post('/api/calculate', async (req, res) => {
    ...
    if (!params.avgMonthlyKwh && !params.avgMonthlyBill && (!params.appliances || params.appliances.length === 0)) {
        return res.status(400).json({ message: 'Please provide energy usage (Avg. kWh, Avg. Bill, or appliances).' });
    }
    ...
    if (params.systemType !== 'on-grid' && (!params.depthOfDischarge || params.depthOfDischarge <= 0 || params.depthOfDischarge > 1)) {
        return res.status(400).json({ message: 'Valid Depth of Discharge (0-1, e.g., 0.8) is required.' });
    }
    ...
    try {
        ...
        const pvSystemDerateFactor = 1.0; // No additional derating beyond PVGIS
        ...
        const actualPvSizeKwP = (numberOfPanels * panelWattage) / 1000;
        ...
        if (params.systemType !== 'on-grid') {
            ...
            console.log(`Battery Actual Size: ${batterySizeKwh.toFixed(2)} kWh (${numberOfBatteries} x ${batteryUnitCapacityKwh} kWh)`);
            ...
            if (revisedNumberOfPanels > numberOfPanels) {
                console.log(`Increasing PV size for Off-Grid: ${numberOfPanels} -> ${revisedNumberOfPanels} panels`);
        ...
        const totalBatteryCost = (params.systemType !== 'on-grid') ? numberOfBatteries * batteryUnitCapacityKwh * prices.batteryCostPerKwh : 0;
...
app.post('/api/generate-pdf', (req, res) => {
    ...
    doc.fontSize(8).text(`Report generated on ${new Date().toLocaleDateString()}. Estimates based on PVGIS and mock pricing. Actual results may vary.`, 50, doc.page.height - 60, { align: 'center', width: doc.page.width - 100 });
    doc.end();
});
