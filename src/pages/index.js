...
function HomePage() {
    ...
    const onDropAccepted = useCallback(async (acceptedFiles) => {
        ...
        try {
            ...
            const datePattern = /\d{2}\/\d{2}\/\d{4}|\d{4}-\d{2}-\d{2}|(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)\s+\d{1,2},\s+\d{4}/i;
            ...
            const consumptionKwh = consumptionStr ? parseFloat(consumptionStr.match(/\d{1,5}(?:\.\d+)?/)[0].replace(/,/g, '')) : null;
            const amountStr = extractField(text, amountLabels, amountPattern);
            const totalAmount = amountStr ? parseFloat(amountStr.replace(/KES/i, '').replace(/,/g, '').trim()) : null;
            ...
            if (!location && billLocation) setLocation(`${billLocation}, Nairobi, Kenya`); // Add context
            ...
            <section style={sectionStyle}>
                ...
                {extractedData && (
                    <div style={ocrResultStyle}>
                        ...
                        <p><strong>Issue Date:</strong> {extractedData.issueDate || 'Not Found'}</p>
                        <p><strong>Customer Name:</strong> {extractedData.customerName || 'Not Found'}</p>
                        <p><strong>Location:</strong> {extractedData.billLocation || 'Not Found'}</p>
                        <div style={formGroupStyle}>
                            <label style={labelStyle}>Detected Avg. Monthly kWh:</label>
                            <input type="number" style={inputStyle} value={avgMonthlyKwh} onChange={(e) => setAvgMonthlyKwh(e.target.value)} placeholder="Enter kWh if not found/incorrect" />
                        </div>
                        <div style={formGroupStyle}>
                            <label style={labelStyle}>Detected Avg. Monthly Bill (KES):</label>
                            <input type="number" style={inputStyle} value={avgMonthlyBill} onChange={(e) => setAvgMonthlyBill(e.target.value)} placeholder="Enter Bill Amount if not found/incorrect" />
                        </div>
            ...
            <section style={sectionStyle}>
                ...
                <div style={gridStyle}>
                    <fieldset style={fieldsetBorderStyle}>
                        ...
                        <div style={formGroupStyle}><label style={labelStyle} htmlFor="location">Project Location:*</label><input id="location" value={location} onChange={(e) => setLocation(e.target.value)} style={inputStyle} placeholder="e.g., Nairobi, Kenya" required /></div>
                        <div style={formGroupStyle}><label style={labelStyle} htmlFor="systemType">System Type:*</label><select id="systemType" value={systemType} onChange={(e) => setSystemType(e.target.value)} style={selectStyle} required><option value="on-grid">On-Grid</option><option value="off-grid">Off-Grid</option><option value="hybrid">Hybrid</option></select></div>
                        <div style={formGroupStyle}><label style={labelStyle} htmlFor="userType">User Type:*</label><select id="userType" value={userType} onChange={(e) => { setUserType(e.target.value); setAppliances([]); }} style={selectStyle} required><option value="residential">Residential</option><option value="commercial">Commercial</option><option value="industrial">Industrial</option></select></div>
                    </fieldset>
                    <fieldset style={fieldsetBorderStyle}>
                        ...
                        <div style={formGroupStyle}><label style={labelStyle} htmlFor="avgMonthlyKwh">Avg. Monthly kWh:</label><input id="avgMonthlyKwh" type="number" value={avgMonthlyKwh} onChange={(e) => setAvgMonthlyKwh(e.target.value)} style={inputStyle} placeholder="From bill or estimate" /></div>
                        <div style={formGroupStyle}><label style={labelStyle} htmlFor="avgMonthlyBill">Avg. Monthly Bill (KES):</label><input id="avgMonthlyBill" type="number" value={avgMonthlyBill} onChange={(e) => setAvgMonthlyBill(e.target.value)} style={inputStyle} placeholder="From bill or estimate" /></div>
                        <div style={formGroupStyle}><label style={labelStyle} htmlFor="electricityPrice">Current Electricity Price (KES/kWh):*</label><input id="electricityPrice" type="number" step="0.1" value={electricityPricePerKwh} onChange={(e) => setElectricityPricePerKwh(e.target.value)} style={inputStyle} placeholder="e.g., 25.5" required /><small style={{ fontSize: '0.8em', color: '#666', marginTop: '3px' }}>Needed for payback calculation.</small></div>
                    </fieldset>
                    {systemType !== 'on-grid' && (
                        <fieldset style={fieldsetBorderStyle}>
                            <legend style={legendStyle}>Battery Details</legend>
                            <div style={formGroupStyle}><label style={labelStyle} htmlFor="autonomyDays">Days of Autonomy:*</label><input id="autonomyDays" type="number" value={autonomyDays} onChange={(e) => setAutonomyDays(e.target.value)} style={inputStyle} min="1" required /><small style={{ fontSize: '0.8em', color: '#666', marginTop: '3px' }}>Days battery lasts without sun.</small></div>
                            <div style={formGroupStyle}><label style={labelStyle} htmlFor="dod">Battery Depth of Discharge (DoD):*</label><input id="dod" type="number" step="0.05" min="0.1" max="1.0" value={depthOfDischarge} onChange={(e) => setDepthOfDischarge(e.target.value)} style={inputStyle} required /><small style={{ fontSize: '0.8em', color: '#666', marginTop: '3px' }}>e.g., 0.8 for 80% DoD</small></div>
                        </fieldset>
                    )}
                    <fieldset style={fieldsetBorderStyle}>
                        <legend style={legendStyle}>Panel Configuration</legend>
                        <div style={formGroupStyle}><label style={labelStyle} htmlFor="panelWattage">Panel Wattage (Wp):*</label><input id="panelWattage" type="number" value={panelWattage} onChange={(e) => setPanelWattage(e.target.value)} style={inputStyle} min="100" step="5" placeholder="e.g., 450" required /><small style={{ fontSize: '0.8em', color: '#666', marginTop: '3px' }}>Wattage of individual panels.</small></div>
                        <div style={formGroupStyle}><label style={labelStyle} htmlFor="tilt">Panel Tilt (degrees):</label><input id="tilt" type="number" value={tilt} onChange={(e) => setTilt(e.target.value)} style={inputStyle} min="0" max="90" placeholder="Optimal near latitude" /></div>
                        <div style={formGroupStyle}><label style={labelStyle} htmlFor="azimuth">Panel Azimuth (degrees):</label><input id="azimuth" type="number" value={azimuth} onChange={(e) => setAzimuth(e.target.value)} style={inputStyle} min="0" max="359" placeholder="180 = South" /></div>
                        <div style={formGroupStyle}><label style={labelStyle} htmlFor="shading">Shading Losses (%):</label><input id="shading" type="number" value={shading} onChange={(e) => setShading(e.target.value)} style={inputStyle} min="0" max="100" /><small style={{ fontSize: '0.8em', color: '#666', marginTop: '3px' }}>% yearly loss due to shade.</small></div>
                    </fieldset>
                    <fieldset style={fieldsetBorderStyle}>
                        <legend style={legendStyle}>Optional Details</legend>
                        <div style={formGroupStyle}><label style={labelStyle}>Budget (KES) (Optional):</label><input type="number" value={budget} onChange={(e) => setBudget(e.target.value)} style={inputStyle} placeholder="Max budget if applicable" /></div>
                        <div style={formGroupStyle}><label style={labelStyle}>Roof Area (sqm) (Optional):</label><input type="number" value={roofArea} onChange={(e) => setRoofArea(e.target.value)} style={inputStyle} placeholder="For future space checks" /></div>
                    </fieldset>
                    <fieldset style={{ ...fieldsetBorderStyle, ...fullWidthStyle }}>
                        ...
                        {appliances.map((appliance, index) => (
                            <div key={index} style={{ display: 'grid', gridTemplateColumns: appliance.name === 'custom' ? '2fr 1fr 1fr 0.5fr 0.5fr auto' : '3fr 1fr 0.5fr 0.5fr auto', gap: '10px', marginBottom: '10px', alignItems: 'center', paddingBottom: '10px', borderBottom: '1px dashed #eee' }}>
                                <select value={appliance.name} onChange={(e) => updateAppliance(index, 'name', e.target.value)} style={{ ...selectStyle }}><option value="custom">-- Custom Appliance --</option>{(applianceCategories[userType] || []).map(app => <option key={app.name} value={app.name}>{app.name} ({app.power}W)</option>)}</select>
                                {appliance.name === 'custom' && (<input type="text" placeholder="Appliance Name" value={appliance.customName || ''} onChange={(e) => updateAppliance(index, 'customName', e.target.value)} style={inputStyle} />)}
                    ...
                    <div style={fullWidthStyle}>
                        <button onClick={handleCalculateClick} disabled={calculating} style={calculating ? buttonDisabledStyle : { ...buttonStyle, backgroundColor: '#28a745', width: '100%', padding: '15px', fontSize: '1.2em' }}>{calculating ? 'Calculating...' : 'Calculate System Size'}</button>
                ...
                {!calculating && calculationResult && !calculationError && (
                    <div style={resultBoxStyle}>
                        ...
                        {calculationResult.budgetConstrained && (
                            <p style={{ color: 'orange', fontWeight: 'bold', marginTop: '10px' }}>Note: System size reduced to meet budget of KES {calculationResult.targetBudget?.toLocaleString()}.</p>
                        )}
                                ...
                                <Bar
                                    data={{
                                        ...
                                        datasets: [{
                                            ...
                                            data: calculationResult.monthlyProduction.map(m => m.production.toFixed(0)),
